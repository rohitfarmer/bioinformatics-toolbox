...
DEPENDENCIES    : hmmsearch must be done against refseq database.
...
while( <> ) {
	...
	# Push everything cached above in an array
	if( $fa && $seq ) {
		$seq =~ s/\-//g;
...
# Sort all the sequences according to length in decreasing order
